#!/usr/bin/env node
/**
 * 言道 · 离线资源打包器 v2.1 (Optimized Edition)
 * 性能优化：Set去重替代Array.find，O(n²)→O(n)
 * 目标：生成200MB+的真实学习数据
...
function genJaVocab(count) {
  ...
  while (result.length < count) {
    ...
    if (!jaWordSet.has(newWord)) {
      jaWordSet.add(newWord);
      result.push({
...
function genEnVocab(count) {
  ...
  while(result.length < count){
    ...
    const vars=[bw+'ing',bw+'s',bw+'ed',bw+'ly','re-'+bw,'un-'+bw,
      'over-'+bw,'under-'+bw,bw+'-ness',bw+'-ment',bw+'-tion',bw+'-ity'];
    const nw=pick(vars);
    if(!enWordSet.has(nw)){
      enWordSet.add(nw);
